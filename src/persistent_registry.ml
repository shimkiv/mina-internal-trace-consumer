...
let add_block_trace ~source block_id =
  ...
  in
  let%map block_trace_id = Store.add_block_trace block_id trace in
  ignore
...
let get_distributions () =
  let open Deferred.Result.Let_syntax in
  let%map distributions = Store.get_value "checkpoint_distributions" in
...
let get_block_traces block_id = Store.get_block_traces block_id
