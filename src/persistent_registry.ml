...
let set, get =
  let dbconn = ref None in
  let set_db db = dbconn := Some db in
  let get_db () =
    Option.value_exn ~message:"Database not initialized" !dbconn
  in
  (set_db, get_db)
...
let push_checkpoint block_trace_id ~is_main ~source ?(call_id=0) ~name ~timestamp ()
    =
  let db = get () in
  Store.add_block_trace_checkpoint db block_trace_id is_main source call_id
...
let push_block_metadata block_trace_id ~metadata =
  let open Deferred.Result.Let_syntax in
  let db = get () in
  let%bind _, trace = Store.get_block_trace_by_id db block_trace_id in
  ...
  in
  Store.update_block_trace db block_trace_id trace
...
let set_produced_block_state_hash block_trace_id state_hash =
  ...
  @@ String.Table.add block_id_trace_id_mapping ~key:state_hash
       ~data:block_trace_id ;
  Store.update_block_trace_block_id db block_trace_id state_hash

let get_distributions () =
  let open Deferred.Result.Let_syntax in
  let db = get () in
  let%map distributions = Store.get_value db "checkpoint_distributions" in
...
let save_distributions distributions =
  let db = get () in
  Store.set_value db "checkpoint_distributions"
  ...
  | status, Some old_status
    when not @@ Block_trace.equal_status old_status status ->
      ...
      let%bind _block_id, trace =
        Store.get_block_trace_by_id db block_trace_id
      in
      let%bind checkpoints =
        Store.get_block_trace_checkpoints db ~main_trace:true block_trace_id
      in
      let timestamps =
        checkpoints
        |> List.filter_map ~f:(function
             | { source = `Main
               ; checkpoint = `Checkpoint (_, t)
               ; call_id = _
               } ->
                 Some t
             | _ ->
      ...
      let%bind () = Store.update_block_trace db block_trace_id trace in
      ignore
...
let get_block_trace_other_checkpoints block_trace_id =
  let db = get () in
  Store.get_block_trace_checkpoints db ~main_trace:false block_trace_id

let get_block_traces block_id =
  let db = get () in
  Store.get_block_traces db block_id

let get_all_block_traces ?max_length ?offset ?height ?global_slot ?chain_length
    ?order () =
  let db = get () in
  Store.get_block_trace_info_entries db ?max_length ?offset ?height ?global_slot
    ?chain_length ?order ()
